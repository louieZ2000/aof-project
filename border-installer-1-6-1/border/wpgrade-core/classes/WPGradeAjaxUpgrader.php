...
class WPGradeAjaxUpgrader {
	...
	function is_update_available( $theme_name, $allow_cache = false ) {
		...
		if ( $installed_theme == null ) {
			$transalated_message = __( 'The theme <b>:theme_name</b> is not installed.<br><i>If you are certain the theme it is installed, please contact support.</i>', 'border_txtd' );
		...
		if ( $errors !== null ) {
			$this->errors = array( __( 'Failed to retrieve theme list via Envato API.', 'border_txtd' ) );
		...
		foreach ( $purchased_themes as $purchased ) {
			if ( $this->is_matching_themes( $installed_theme, $purchased ) ) {
				if ( $this->is_newer_version_available( $installed_theme['Version'], $purchased->version ) ) {
					...
					$transalated_message = __( 'There is no update available for the theme <b>:theme_name</b>', 'border_txtd' );
					$this->errors        = array( strtr( $transalated_message, array( ':theme_name' => $theme_name ) ) );
		...
		$transalated_message = __( 'Failed to find the theme <b>:theme_name</b>', 'border_txtd' );
		$this->errors        = array( strtr( $transalated_message, array( ':theme_name' => $theme_name ) ) );
	...
	function download_url( $theme_name, $allow_cache = true ) {
		...
		if ( ! empty( $internal_errors ) || $purchased_themes === false ) {
			$transalated_message = __( 'TeamForest/Envato API error.', 'border_txtd' );
		...
		if ( $marketplace_theme_data == null ) {
			$transalated_message = __( 'Failed to find <b>:theme_name</b> in your list of purchased themes. (please contact support)', 'border_txtd' );
			$this->errors        = array( strtr( $transalated_message, array( ':theme_name' => $theme_name ) ) );

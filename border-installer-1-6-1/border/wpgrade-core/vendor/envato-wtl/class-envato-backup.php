...
class Envato_Backup {
	...
	public function check_archive() {
		...
		if ( ! file_exists( $this->archive_filepath() ) ) {
			$this->error( $this->archive_method(), __( 'The backup file was not created', 'border_txtd' ) );
		}
	...
	public function files() {
		...
		if ( ! empty( $this->unreadable_files ) ) {
			$this->warning( $this->archive_method(), __( 'The following files are unreadable and could not be backed up: ', 'border_txtd' ) . implode( ', ', $this->unreadable_files ) );
		}

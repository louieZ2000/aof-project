...
if ( ! empty( $attachments ) ) :
	...
	//if the gallery has a custom excerpt we will use that for sharing - we will leave it the metas only for now
	?>
	<div class="mosaic-wrapper">
		<div
			class="mosaic  mosaic--masonry  js-gallery  js-mosaic--masonry" <?php echo ( $share_button ) ? 'data-share-buttons="' . $share_links . '"' : '' ?>>
			<?php
			...
			foreach ( $attachments as $key => $attachment ):
				...
				$thumb_img = wp_get_attachment_image_src( $attachment->ID, 'small-size' );
				$full_img  = wp_get_attachment_image_src( $attachment->ID, 'full-size', false );
				...
				<div
					class="mosaic__item<?php echo ( ! empty( $video_url ) ) ? ' item--video' : ''; ?>  <?php echo ( ! empty( $external_url ) ) ? ' item--link' : ''; ?>">
					<a href="<?php echo $the_link ?>" <?php if ( ! empty( $external_url ) ) {
						echo ' target="_blank" ';
					   ...
					   title="<?php echo $attachment->post_title; ?>">
						<div class="mosaic__image" style="padding-top: <?php echo $thumb_img_ratio; ?>%;">
							<img src="<?php echo $thumb_img[0]; ?>" data-src="<?php echo $full_img[0]; ?>" alt="">
						</div>
						<div class="mosaic__meta">

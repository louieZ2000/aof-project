...
| (?<=[\xF0-\xF7][\x80-\xBF])[\x80-\xBF](?![\x80-\xBF]) # Short 4 byte sequence (2)
   ...
   static function get_content_link_url() {
    ...
    static function get_archive_title() {
		...
		if ( is_home() ) {
            ...
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Search Results for: ', 'border_txtd' ) ?>
                </span> <?php echo get_search_query(); ?>
            </h3>
        <?php
        } elseif ( is_tag() ) { ?>
                <h3 class="blog-archive-title">
                    <span>
                        <?php _e('Tag Archives: ', 'border_txtd' ) ?>
                    </span> <?php echo single_tag_title( '', false ); ?>
                </h3>
        <?php } elseif (!empty($object) && isset($object->term_id)) { ?>
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Blog Category: ', 'border_txtd' ) ?>
                </span> <?php echo $object->name; ?>
            </h3>
        <?php } elseif ( is_day() ) {?>
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Daily Archives: ', 'border_txtd' ) ?>
                </span> <?php echo get_the_date(); ?>
            </h3>
        <?php } elseif ( is_month() ) { ?>
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Monthly Archives: ', 'border_txtd' ) ?>
                </span> <?php echo get_the_date( _x( 'F Y', 'monthly archives date format', 'border_txtd' ) ) ; ?>
            </h3>
        <?php } elseif ( is_year() ) { ?>
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Yearly Archives: ', 'border_txtd' ) ?>
                </span> <?php echo get_the_date( _x( 'Y', 'yearly archives date format', 'border_txtd' ) ) ; ?>
            </h3>
        <?php } else { ?>
            <h3 class="blog-archive-title">
                <span>
                    <?php _e('Archives', 'border_txtd' ) ?>
                </span>
            </h3>
        <?php }
    }
			...
			<div class="flexbox  archive-categories-wrapper">
				<div class="flexbox__item">
					...
					<div class="pix-dropdown  up  border-menu">
						<a class="dropdown__trigger"  href="#"><?php echo __('Filter','border_txtd') ?>  <span class="arrow"></span></a>
						<ul class="dropdown__menu  nav  nav--banner  archive-categories-list  border-menu">
							<?php foreach($filter_categories as $key=>$category) { ?>
								<li class="category-item archive__filter-item"><a href="#" title="<?php echo sprintf( __( "View all in %s" ,'border_txtd'), $category->name ); ?>" data-filter="<?php echo '.cat-'.$category->slug; ?>"><?php echo $category->name; ?></a></li>
							<?php } ?>
							<li class="category-item archive__filter-item" data-filter="all"><a href="#" title="<?php echo __('View all','border_txtd') ?>" data-filter="*"><?php _e('View All', 'border_txtd'); ?></a></li>
						</ul>
					</div>
					<?php else: ?>
					<ul class="dropdown__menu  nav  nav--banner  archive-categories-list  border-menu">
